...
contract AvatarNFT is ERC721, Ownable {
    ...
    constructor(address keeper) ERC721("Crypto HK: Citizens", "CHK") Ownable() {
        _keeper = keeper;
    }
    ...
    modifier onlyOperator() {
        require(
            owner() == msg.sender || _keeper == msg.sender,
    ...
    function build(uint256 tokenId, string memory data) public onlyOperator {
        ...
        require(data.size() > 0, "DO NOT unset any token");

        if (tokenUriData[tokenId].size() > 0) {
            if (data.startsWith("http")) tokenUriData[tokenId] = data;
            else
    ...
    function build(uint256 tokenId, string[] memory list) public onlyOperator {
        require(_keeper != address(0), "Contract is unready, no keeper set");
        require(
            tokenId > 0 && tokenId <= MAX_TOKEN_ID,
            "token id out of range"
        );
        require(tokenId + list.length <= MAX_TOKEN_ID, "token id out of range");
        require(list.length > 0, "data list of tokenURI is required");
        require(list.length < 32, "act with caution");
        for (uint256 i = 0; i < list.length; i++) {
            if (list[i].size() < 1) continue;
            uint256 ntid = tokenId + i;
            if (tokenUriData[ntid].size() > 0) {
                if (list[i].startsWith("http")) tokenUriData[tokenId] = list[i];
                else
                    tokenUriData[ntid] = string(
                        abi.encodePacked(
                            "data:application/json;base64,",
                            list[i]
                        )
                    );
                emit AvatarCreated(ntid);
            } else {
                if (list[i].startsWith("http")) tokenUriData[tokenId] = list[i];
                else
                    tokenUriData[ntid] = string(
    ...
    function mint(uint256 tokenId, string memory data) public onlyOperator {
        ...
        if (tokenUriData[tokenId].size() > 0) {
            ...
            require(data.size() > 0, "data of tokenURI is required");
            if (data.startsWith("http")) tokenUriData[tokenId] = data;
            else
                tokenUriData[tokenId] = string(
                    abi.encodePacked("data:application/json;base64,", data)
